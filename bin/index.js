...
async function login() {
  ...
  try {
    ...
    const savePath = loginDetails.qr
      ? path.join(QR_TEMP_DIR, `qr-code-${Date.now()}.jpeg`)
      : null;
...
async function checkAuthStatus() {
  ...
  try {
    ...
    const cookiesString = await fs.readFile(COOKIE_PATH, "utf-8").catch(() => null);
    if (!cookiesString) return { status: 2 };
